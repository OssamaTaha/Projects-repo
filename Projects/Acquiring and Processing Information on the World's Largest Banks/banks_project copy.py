...
def extract(url):
    ...
    # Filter out non-numeric market caps
    df = df[df['MC_USD_Billion'].apply(lambda x: x.replace('$', '').replace('B', '').replace('.', '', 1).isdigit())]
...
# Define the required entities and call the relevant functions in the correct order to complete the project.
